...
async function resetAllPoints() {
  ...
  try {
    ...
    const [beforeUsers] = await connection.query(
      "SELECT COUNT(*) as total, COALESCE(SUM(points), 0) as total_points, COALESCE(MAX(points), 0) as max_points FROM users"
    );
    console.log("users表:", beforeUsers[0]);
    
    const [beforeChildren] = await connection.query(
      "SELECT COUNT(*) as total, COALESCE(SUM(points), 0) as total_points, COALESCE(MAX(points), 0) as max_points FROM child_profiles"
    );
    console.log("child_profiles表:", beforeChildren[0]);
    ...
    console.log(`✓ users表已更新，影响行数: ${usersResult.affectedRows}`);
    
    // 更新child_profiles表
    const [childrenResult] = await connection.query("UPDATE child_profiles SET points = 0");
    console.log(`✓ child_profiles表已更新，影响行数: ${childrenResult.affectedRows}`);
    ...
    console.log("users表:", afterUsers[0]);
    
    const [afterChildren] = await connection.query(
      "SELECT COUNT(*) as total, COALESCE(SUM(points), 0) as total_points, COALESCE(MAX(points), 0) as max_points FROM child_profiles"
    );
    console.log("child_profiles表:", afterChildren[0]);
