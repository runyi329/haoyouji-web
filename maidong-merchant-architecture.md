...
商家本人打开自己的网站时，系统自动识别为已登录状态（通过 localStorage + refresh token 实现长期保持）。**除非商家主动点击退出，否则始终保持登录态**。人脉按钮进入商家自己的人脉数据，钱脉按钮进入商家自己的钱脉账本，个人中心可管理商品、查看订单。

**模式B：陌生访客访问（默认开放浏览，按需登录）**

> **核心规则**：全站所有浏览类页面（首页、品牌中心、商品详情页）对未登录用户**完全开放**，不做任何登录检查、不弹出登录提示、不遮挡内容。无论用户是通过分享链接还是直接输入网址访问，都能直接看到完整页面内容。

陌生人打开链接后，底部导航与商家本人看到的**完全一样**（三个按钮）。只有以下操作才触发登录跳转：

- 点击底部导航的「人脉」按钮 → 跳转登录页
- 点击底部导航的「钱脉」按钮 → 跳转登录页
- 点击商品页的「购买」「加入购物车」按钮 → 跳转登录页
- 点击页面上的「登录」按钮 → 跳转登录页
...
| 访问者 | 打开方式 | 浏览页面 | 点击人脉/钱脉/购买 | 登录持久性 |
|--------|---------|---------|------------------|----------|
| 商家本人 | 直接打开自己的网址 | 直接看到全部内容 | 直接进入对应功能 | 长期保持，除非主动退出 |
| 陌生访客 | 通过分享链接或直接访问 | 直接看到全部内容（免登录） | 触发登录跳转，登录后进入访客自己的数据 | 登录后保持，关闭浏览器失效 |

**关键规则**：底部导航对所有人完全一样，区别只在于登录状态和数据归属。浏览类操作（看首页、看商品、看详情）永远不要求登录；交互类操作（购买、进入人脉/钱脉）才要求登录。
...
|----------|------|
| 已登录 | 正常显示商家首页，所有功能可用 |
| 未登录 | **正常显示商家首页**，浏览内容完全开放；点击人脉/钱脉/购买时才跳转登录页 |
| 加载中 | 显示加载骨架屏（避免闪烁） |
...
所有分享链接必须自动携带**当前登录用户的邀请码**，格式为 `?ref={当前用户inviteCode}`。未登录时使用商城默认邀请码（如 `cx8618`）。

示例：`https://jiangyuchen.cn/wine/product/romanico?ref=B的邀请码`

**`ref=` 参数的唯一作用是推荐注册绑定**，与订单归属无关：

| 客人状态 | 处理逻辑 |
|---------|----------|
| 已有账号（已注册用户） | 正常登录，不重复绑定邀请关系，登录后返回原页面 |
| 新用户（未注册） | 跳转到注册页，自动预填邀请码，注册完成后该客人自动成为**分享者**的人脉 |

**订单归属规则（铁律）**：

订单归属由**商品所在的商城**决定，与分享者无关。具体规则如下：

| 场景 | 订单归属 | 佣金归属 |
|------|---------|----------|
| A 分享自己商城的商品链接 | A（商城拥有者） | A |
| B 分享 A 商城的商品链接 | A（商城拥有者） | 不计算 B 的佣金 |
| B 有自己的商城，B 的商城里也上了同款商品，B 分享自己商城的链接 | B | B |

**核心原则**：想获得佣金，必须在自己的商城里上架商品，分享自己商城的链接。分享他人商城的链接只能获得推荐注册关系，不产生佣金。

**分享面板 UI 规则**：
- 点击分享按钮弹出分享面板时，面板顶部必须显示当前用户的邀请码，例如：「已包含您的邀请码 **cx8618**，好友注册后自动成为您的人脉」
- 未登录用户分享时显示：「好友注册后将绑定本商城」

**实现要点**：
1. 分享按钮生成链接时，读取 `useAuth().user?.inviteCode`，动态拼接 `?ref={inviteCode}`
2. 未登录时使用商城默认邀请码（硬编码在各商城页面中）
3. 系统在 `localStorage` 中存储 `ref` 参数（有效期 7 天），防止用户中途关闭页面后丢失邀请关系
4. 注册页读取 `localStorage` 中的 `ref` 并预填到邀请码字段（用户不可修改）
5. 注册成功后清除 `localStorage` 中的 `ref` 记录
...
```
用户访问商家首页 / 或 /wine 或 /beauty
    ↓
检查认证状态
    ├── loading = true → 显示加载骨架屏（避免闪烁）
    ├── isAuthenticated = false → 直接显示首页内容（免登录浏览）
    └── isAuthenticated = true → 显示首页内容，全功能可用
                                    ├── username = 'liulifan' → 跳转 /beauty（每次会话一次）
                                    └── 其他用户 → 正常显示首页

未登录用户点击下列按钮时才跳转登录页：
    ├── 点击「人脉」按钮 → 跳转 /login?redirect=当前路径
    ├── 点击「钱脉」按钮 → 跳转 /login?redirect=当前路径
    ├── 点击商品「购买」/「加入购物车」 → 跳转 /login?redirect=当前路径
    └── 点击「登录」按钮 → 跳转 /login
```
...
| 路由 | 访问权限 | 说明 |
|------|---------|------|
| `/` | 公开 | 人脉首页（未登录可浏览，点击人脉/钱脉时要登录） |
| `/login` | 公开 | 登录页 |
| `/register` | 公开 | 注册页（自动读取 `?ref=` 邀请码） |
| `/{merchantCode}` | 公开 | 商家首页（完全免登录浏览） |
| `/{merchantCode}/brands` | 公开 | 商品列表（完全免登录浏览） |
| `/{merchantCode}/product/:slug` | 公开 | 商品详情页（免登录浏览，购买时要登录） |
| `/share/{merchantCode}/product/:slug` | 公开 | 商品分享页（海报式，无导航栏，购买时要登录） |
| `/{merchantCode}/profile` | 公开（操作需登录） | 个人中心 |
...
商品在系统中同时拥有两个独立的访问链接：**商品详情页**（内部使用，需登录）和**商品分享页**（对外传播，无需登录）。分享按钮生成的链接始终指向分享页，确保接收者无需注册即可浏览完整商品信息，降低传播摩擦。

### 24.1 双链接定义

| 属性 | 商品详情页 | 商品分享页 |
|------|-----------|------------|
| 路由模式 | `/wine/product/:slug` 或 `/:merchantCode/product/:id` | `/share/product/:slug` 或 `/share/:merchantCode/product/:id` |
| 登录要求 | **需要登录**（未登录自动跳转登录页） | **无需登录**（任何人可直接访问） |
| 顶部导航 | 有返回按钮、购物车入口 | **无返回按钮**、无顶部导航栏 |
| 页面性质 | App 内部页面，嵌入在导航体系中 | 独立海报式页面，脱离 App 导航体系 |
| 购买行为 | 直接加入购物车 / 立即购买 | 点击购买按钮时才跳转登录页，登录后自动回到商品详情页 |
| 分享按钮 | 生成分享页链接（非详情页链接） | 可继续分享（生成同一分享页链接） |
| 使用场景 | 用户在 App 内浏览、下单 | 微信/朋友圈/短信等外部渠道传播 |

### 24.2 分享页设计原则

分享页的定位是**电子海报**，核心目标是让接收者在最短时间内了解商品全貌并产生购买意愿。因此需遵循以下原则：

1. **零门槛访问**：分享页不做任何登录检查，不弹出登录提示，不遮挡内容。所有商品信息（图片、价格、描述、评分等）完整展示，与详情页内容一致。

2. **独立页面结构**：分享页不包含 App 的顶部导航栏、底部 TabBar、返回按钮等 App 内部导航元素。页面顶部可展示商家 Logo 或品牌标识，但不能有 App 级别的 UI 元素。

3. **唯一行动入口**：页面底部固定一个醒目的购买按钮（如「立即购买」或「加入购物车」）。点击此按钮时，若用户未登录则跳转到登录页，登录成功后自动回到**商品详情页**（非分享页），进入正常购买流程。

4. **可二次分享**：分享页本身也包含分享功能，接收者可以继续将该页面分享给其他人，形成裂变传播链。

### 24.3 路由规范

```
商品详情页（需登录）：
  /wine/product/romanico          ← 静态路由（已制作详情页的商品）
  /wine/product/:id               ← 动态路由（通用商品）
  /:merchantCode/product/:id      ← 跨商家通用路由

商品分享页（无需登录）：
  /share/wine/product/romanico    ← 静态路由对应的分享页
  /share/wine/product/:id         ← 动态路由对应的分享页
  /share/:merchantCode/product/:id ← 跨商家通用分享页
```

分享页路由统一以 `/share/` 前缀开头，后续路径与详情页保持一致，便于代码中相互转换。

### 24.4 前端实现要求

1. **组件复用**：分享页与详情页共享同一套商品展示组件（`ProductImageCarousel`、`ProductPriceTitle`、`ProductDetailImages` 等），但分享页不加载 `ProductActionBar`（吸底购买栏），改为使用专用的 `SharePageActionBar` 组件。

2. **SharePageActionBar 组件**：底部固定的购买按钮，点击后执行以下逻辑：检查用户是否已登录（通过 `useAuth()`）；已登录则跳转到对应的商品详情页（`/wine/product/:slug`）；未登录则跳转到登录页，登录成功后自动回到商品详情页。

3. **路由注册**：在 `App.tsx` 中，分享页路由使用 `publicProcedure`（无需鉴权），详情页路由使用 `protectedProcedure`（需要鉴权）。

4. **分享链接生成**：所有商品页面的分享按钮，生成的链接必须是分享页路由（`/share/...`），而非详情页路由。分享链接应包含完整域名，格式如：`https://jiangyuchen.cn/share/wine/product/romanico`。
...
| 商家自由装修区 | 有 | 有 |
| 详情图 | 有 | 有 |
| 吸底购买栏 | 有（直接购买） | 有（点击后跳转登录/详情页） |
| 分享按钮 | 有（生成分享页链接） | 有（生成同一分享页链接） |
| 评论区 | 有 | 无（或只读展示） |

### 24.6 开发注意事项

1. **SEO 与社交分享**：分享页应注入正确的 Open Graph / 微信分享 Meta 标签（标题、描述、缩略图），确保在微信、朋友圈等平台分享时显示商品卡片预览。

2. **数据接口**：分享页使用 `publicProcedure` 查询商品数据，不要求 `ctx.user`。接口应返回与详情页相同的商品数据结构。

3. **埋点追踪**：分享页应记录访问来源（如 `?from=wechat&sharer=userId`），用于统计分享转化率。

4. **防滥用**：分享页虽然无需登录，但应有基本的频率限制，防止爬虫批量抓取商品数据。
