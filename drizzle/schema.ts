...
export const afAdvancedOrders = mysqlTable("af_advanced_orders", {
  ...
  refundBalanceId: int('refund_balance_id'),
...
export const ethPositionLevels = mysqlTable("eth_position_levels", {
  ...
  baseNotes: text('base_notes'),       // 底仓备注（JSON字符串，格式：[{text,time}]）
  tacticalNotes: text('tactical_notes'), // 机动仓备注（JSON字符串，格式：[{text,time}]）
  updatedAt: timestamp('updated_at', { mode: 'string' }).defaultNow().onUpdateNow().notNull(),
  createdAt: timestamp('created_at', { mode: 'string' }).default('CURRENT_TIMESTAMP').notNull(),
}, (table) => [
...
export const ethPositionChangeLogs = mysqlTable("eth_position_change_logs", {
  ...
  changeType: mysqlEnum('change_type', ['actual', 'planned', 'notes']).notNull(),   // 修改类型：已买/计划/备注
  ...
  note: text('note').notNull(),           // 用户可编辑的备注（notes类型时存储完整JSON）
  createdAt: timestamp('created_at', { mode: 'string' }).default('CURRENT_TIMESTAMP').notNull(),
  updatedAt: timestamp('updated_at', { mode: 'string' }).defaultNow().onUpdateNow().notNull(),
}, (table) => [
...
export const mibanCommissionConfig = mysqlTable("miban_commission_config", {
  ...
  planId: int("plan_id"),
  payoutRateMultiplier: decimal("payout_rate_multiplier", { precision: 5, scale: 4 }).default('1.0000').notNull(),
  note: text("note"),
