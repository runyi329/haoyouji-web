...
export default function CustomAGManager() {
  ...
  const { data: agLedgers, isLoading } = trpc.ledger.listCustomAG.useQuery();

  const createMutation = trpc.ledger.createCustomAG.useMutation({
    onSuccess: () => {
      toast.success("AG 定制账本创建成功");
      setShowCreate(false);
      setNewName("");
      setNewDesc("");
      utils.ledger.listCustomAG.invalidate();
    },
    onError: (e) => toast.error(e.message),
  });

  const inviteMutation = trpc.ledger.inviteToCustomAG.useMutation({
    onSuccess: (data: any) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="p-4">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-bold text-base">定制账本 (AG) 管理</h2>
          </div>
          <Button
            size="sm"
        ...
        <p className="text-xs text-gray-500">
          AG 型定制账本（共享图片助记词），仅管理员可创建，普通用户在新建账本列表中不可见，只有被邀请后才能进入。
        </p>
      ...
      ) : !agLedgers || agLedgers.length === 0 ? (
        <Card className="p-6 text-center text-gray-400 text-sm">
          暂无 AG 定制账本，点击上方「新建AG账本」创建第一个
        </Card>
      ) : (
        <div className="space-y-3">
          {agLedgers.map((ledger) => (
            <Card key={ledger.id} className="p-4">
              <div className="flex items-start justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    className="flex items-center gap-1 text-xs text-[#D32F2F] hover:text-red-800"
                    onClick={() =>
