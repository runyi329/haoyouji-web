...
只使用表中存在的字段：ts_code, symbol, name, area, industry, market, exchange, list_status, list_date
"""
...
def main():
    ...
    # 只保留表中存在的列
    keep_cols = ['ts_code', 'symbol', 'name', 'area', 'industry', 'market', 'exchange', 'list_status', 'list_date']
    df = df[[c for c in keep_cols if c in df.columns]]
    print(f"获取到 {len(df)} 只退市股票，字段：{list(df.columns)}")
    ...
    try:
        with conn.cursor() as cur:
            ...
            for _, row in df.iterrows():
                rows.append(tuple(
                    None if (str(v) == 'nan' or str(v) == 'None') else v
                    for v in row
                ))
