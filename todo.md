...
- [x] 修改 Home.tsx 底部导航
  - 将当前的 6 个功能按钮改为 5 个新按钮
  - 5 个按钮：首页、发现、消息、动态、我的
  - 使用 lucide-react 线条图标（HomeIcon、Search、MessageCircle、Activity、User）
  - 首页按钮跳转到 /，其他按钮点击显示“功能开发中”
...
- [x] 修改 Home.tsx 底部导航
  ...
  - 点击跳转到 /ledger 页面
- [ ] 实现记账功能
  - 设计数据库表：ledgers（账本表）、ledger_members（成员表）、transactions（账单表）
  - 实现后端 API：账本列表、创建账本、账单列表、添加账单
  - 实现账本列表页面（/ledger）
  - 实现账本详情页面（/ledger/:id）
