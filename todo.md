...
- [ ] 检查 contact_field_categories 表的结构和数据
- [ ] 检查 contact_custom_fields 表的结构和数据
- [ ] 验证扩展信息的数据流程是否正确
...
- [x] 初始化固定的扩展信息类目（50个类目）
- [x] 添加“联系管理员添加新类目”功能（显示微信号 tina_u）
- [x] 支持同一类目多次添加（如3个公司）
- [x] 修改后端 API 支持扩展信息的增删改查
- [x] 修改编辑页面：从类目菜单选择并添加扩展信息
- [x] 修改详情页：在基础信息容器显示所有扩展信息
- [x] 基础信息（姓名、称呼、所在地、性别）只能修改不能删除
- [x] 扩展信息可以修改和删除
- [x] 测试完整流程（所有 Vitest 测试通过）


## 优化扩展信息分类选择方式（树状分类 + 弹出框）✅

- [x] 设计树状分类结构（主分类 + 子分类）
  - 📍 地址：快递地址、公司地址、家庭地址、办公地址、户籍地址、常住地址
  - 📞 联系方式：手机号码、座机号码、微信号、QQ号、邮箱、公司电话、公司邮箱
  - 💼 职业信息：公司名称、职位、部门、工号、行业、入职日期
  - 🎂 个人信息：生日、血型、星座、爱好、学历、毕业院校、专业
  - 🌐 社交账号：个人网站、领英主页、微博、抖音、小红书、Facebook、Twitter、Instagram
  - 💳 其他信息：银行卡号、身份证号、护照号、车牌号、驾驶证号、紧急联系人、紧急联系电话、备注
- [x] 更新数据库表结构支持分类层级（parentCategoryId + icon）
- [x] 创建初始化脚本，添加主分类和子分类数据（6个主分类，42个子分类）
- [x] 修改后端 API 支持返回树状结构
- [x] 创建 FieldCategorySelector 组件：弹出框展示主分类
- [x] 实现主分类点击后展开子分类的交互
- [x] 实现子分类选择后输入字段值
- [x] 优化手机端的弹出框样式和布局
- [x] 更新 AddContact.tsx 使用新的分类选择器
- [x] 测试完整的添加流程
- [x] 更新测试用例（所有测试通过）

### 功能说明
- 6个主分类：地址、联系方式、职业信息、个人信息、社交账号、其他信息
- 42个子分类，覆盖常用的所有字段类型
- 点击主分类展开子分类，选择子分类后输入值
- 支持返回上一级，操作流程清晰
- 手机端体验优化，弹出框自适应屏幕
...
- [x] 诊断问题：点击“AI情报”按钮后只显示历史记录，没有自动触发新的 AI 背调
- [x] 修复 AIBackgroundCheck 组件，使用 hasAutoSearched 状态跟踪自动搜索
- [x] 每次打开侧边栏时自动触发新的 AI 背调查询
- [x] 关闭侧边栏时重置状态，下次打开时会再次触发


## AI 情报功能添加骨架屏加载状态 ✅

- [x] 设计骨架屏 UI（显示“AI 正在分析中...”提示 + 脉动动画）
- [x] 修改 AIBackgroundCheck 组件，在 isLoading 时显示骨架屏
- [x] 添加阶段提示文字（正在搜索公开信息并生成报告，预计需要 10-15 秒）
- [x] 测试加载效果

### 功能说明
- 点击“AI情报”按钮后立即显示骨架屏
- 显示“AI 正在分析中...”提示 + 旋转加载图标
- 骨架屏元素带脉动动画效果
- 预计时间提示：10-15 秒
- 加载完成后自动显示结果
...
- [x] 设计 AI 管理页面功能
  - 提示词管理（System Prompt + User Prompt Template）
  - AI 参数配置（temperature + max_tokens）
- [x] 创建后端 API 支持提示词管理
  - GET /api/ai/prompts - 获取当前提示词
  - PUT /api/ai/prompts - 更新提示词
  - POST /api/ai/prompts/reset - 重置为默认提示词
- [x] 修改 ai-search.ts 使用配置文件中的提示词
- [x] 创建前端 AI 管理页面
  - 提示词编辑器（Textarea）
  - 参数调整（Slider）
  - 保存和重置按钮
  - 可用变量提示
- [x] 在 App.tsx 中添加 AI 管理页面路由
- [x] 在首页下拉菜单添加"AI 管理"入口
- [x] 编写完整的 Vitest 测试用例（所有测试通过）

### 功能说明
- 点击首页头像下拉菜单中的"AI 管理"进入管理页面
- 支持编辑 System Prompt 和 User Prompt Template
- 支持调整 temperature（0.0-2.0）和 max_tokens（100-4000）
- 支持保存配置并立即生效
- 支持重置为默认配置
- 提供可用变量提示（{{name}}, {{company}}, {{position}} 等）

### 待办（后续优化）
- AI 使用统计（调用次数、API 消耗、最近记录）
- 历史记录管理（查看、删除、导出）
