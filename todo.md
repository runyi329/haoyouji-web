...
- [x] 更新脉动学院页面内容
  - 添加 Smartphone 图标
  - 创建 PWA 模块（id: pwa, color: text-pink-600）
  - 添加6个子节内容
- [x] 测试文档页面显示效果
  - PWA 模块正常显示在功能列表中
  - 点击展开后内容完整显示
...
### 文档内容概要
**PWA 桌面应用模块包含：**
1. 什么是 PWA？ - 解释 PWA 概念和优势
2. 安装优势 - 5大优点（桌面图标、全屏体验、离线访问、快速启动、自动更新）
3. Android 手机安装步骤 - 5步详细指南 + 小贴士
4. iPhone/iPad 安装步骤 - 5步详细指南 + Safari 要求说明
5. 常见问题 - 4个 Q&A（卸载、存储空间、联网需求、更新方式）
6. 技术原理 - Service Worker 缓存机制说明
...
- [x] 添加“添加主屏”按钮
  ...
  - 位置：常用功能区域第一位
- [x] 实现点击跳转逻辑
  - 跳转到脉动学院的 PWA 文档部分（/parent/academy#pwa）
  - 在 Academy.tsx 中添加 URL 锤点自动展开功能
  - 为 AccordionItem 添加 id 属性支持锤点定位
- [x] 测试按钮功能
  - 点击按钮正常跳转到脉动学院
  - PWA 模块自动展开
  - 页面自动滚动到 PWA 内容区域
...
- [x] 分析需求并设计手势交互方案
  ...
  - 设计滑动阈值（100px 触发返回）
  - 用户需求：几乎所有子页面都需要此功能
  - 决定：在 DashboardLayout 中全局集成
- [x] 实现右划返回手势组件
  - 创建 useSwipeBack 自定义 Hook
  - 监听 touchstart、touchmove、touchend 事件
  - 计算滑动距离和方向
  - 返回滑动进度供视觉反馈使用
- [x] 全局集成到 DashboardLayout
  - 在 DashboardLayout 中集成 useSwipeBack
  - 添加视觉反馈指示器（左侧圆形返回图标）
  - 智能判断返回路径（首页不启用，子页面返回首页）
  - 只在移动端启用，不影响桌面端
  - 使用 passive: false 防止浏览器默认返回手势
- [x] 测试手势功能
  - 代码已集成，等待用户在真实手机上测试
  - 功能逻辑：屏幕左侧 50px 内开始触摸，向右滑动 100px 触发返回
  - 视觉反馈：滑动时左侧显示返回箭头图标，透明度随滑动进度变化
  - 自动适配：所有使用 DashboardLayout 的页面自动拥有此功能


## 修改右划返回手势功能 - 支持全屏幕滑动✅

- [x] 修改 useSwipeBack Hook
  - 移除左侧边缘限制（edgeWidth 参数和相关检查逻辑）
  - 支持在屏幕任意位置开始滑动（isEdgeSwipe 始终为 true）
  - 保持向右滑动的方向判断（deltaX > 0 且 > deltaY）
  - 保持滑动阈值（100px）
- [x] 更新 DashboardLayout 调用
  - 移除 edgeWidth: 50 参数
  - 保持 threshold: 100 和 enabled 配置
- [x] 测试修改后的功能
  - 代码已修改，等待用户在手机上测试
...
- [x] 修改 Profile.tsx 中的按钮文字
  - 将“添加主屏”改为“安卓主屏”
  - “已安装”保持不变
  - 明确功能适用于安卓设备，iOS 用户一目了然


## 实现页面跟随手指移动的右划返回效果✅

- [x] 修改 DashboardLayout 实现页面跟随
  - 使用 transform: translateX() 让页面跟随手指移动
  - 添加平滑的过渡动画（swipeProgress === 0 时 0.3s ease-out）
  - 达到阈值后松手触发返回（swipeProgress >= 1）
  - 未达到阈值时页面弹回原位（transition 自动处理）
  - 参考抖音的交互效果
- [x] 测试页面跟随效果
  - 代码已实现，等待用户在手机上测试
  - 手指移动时页面应该跟随移动
...
- [x] 修改 DashboardLayout 中的 threshold 参数
  - 从 100px 降低到 80px
  - 同步修改页面移动距离和视觉反馈指示器宽度
  - 让触发返回更容易，提升操作流畅度
...
- [x] 修改布局顺序
  - 移除顶部的 TabsList
  - 在登录按钮下方添加 TabsList（登录和注册表单都添加）
  - 现在顺序：用户名 → 密码 → 登录按钮 → 登录/注册切换按钮
- [x] 测试布局效果
  - 代码已修改，等待用户测试
...
- [x] 查找人脉列表页文件（ContactsList.tsx）
  - 找到 ContactsList.tsx 文件
- [x] 找到并移除返回按钮代码
  - 移除第 808-815 行的返回按钮代码
  - 返回按钮位于共享人脉筛选按钮右侧
- [x] 测试页面显示效果
  - 代码已修改，等待用户测试
