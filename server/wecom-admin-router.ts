  ...
  try {
    ...
    const [rows] = await (conn as any).execute(
      `SELECT
         ...
         u.role           AS user_role,
         (COALESCE(u.balance, 0) + COALESCE((SELECT SUM(amount) FROM af_manual_balances WHERE user_id = u.id AND note NOT LIKE '[CNY]%' AND note NOT LIKE '[BALANCE_BASE]%'), 0)) AS user_balance_usdt,
