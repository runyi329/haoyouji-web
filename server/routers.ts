...
4. 体现故事主题和情节`;
  ...
  ledger: router({
    ...
    // 撤回报销申请（仅限 ajStatus='pending' 且是自己提交的账目）
    withdrawReimbursement: protectedProcedure
      .input(z.object({
        ledgerId: z.number(),
        recordId: z.number(),
      }))
    ...
    funderGetAssetOrders: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        for (const coin of ['BTC', 'ETH', 'SOL', 'AAVE', 'SUI', 'ONDO', 'ASTER', 'LDO', 'ENA', 'ARKM', 'USDT', 'TSLA', 'NVDA', 'AAPL', 'MSFT', 'GOOGL', 'META', 'AMZN', 'SPY', 'QQQ', 'NFLX', 'ORCL', 'TSM', 'AMD', 'CL', 'NG']) {
          const p = getLatestPrice(coin);
    ...
    funderCreateAssetOrder: protectedProcedure
      .input(z.object({
        ledgerId: z.number(),
        userId: z.number(),
        coin: z.enum(['BTC', 'ETH', 'SOL', 'USDT', 'CNY', 'TSLA', 'NVDA', 'AAPL', 'MSFT', 'GOOGL', 'META', 'AMZN', 'SPY', 'QQQ', 'NFLX', 'ORCL', 'TSM', 'AMD', 'CL', 'NG']),
        amount: z.string(),
    ...
    funderGetParticipantsPaymentInfo: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        // 1. 获取订单资金方（funder_asset_orders.user_id 就是资金方的系统 userId）
        const orderRows = await db.execute(
          sql`SELECT o.user_id as funderUserId, u.username, u.name as userName, u.avatar,
                     lm.nickname
        ...
        // 2. 获取订单参与方（funder_order_participants.user_id 直接是系统用户 ID）
        const participantRows = await db.execute(
          sql`SELECT p.id, p.user_id as userId, p.role as participantRole,
                     u.username, u.name as userName, u.avatar,
                     lm.nickname
              FROM funder_order_participants p
              LEFT JOIN users u ON u.id = p.user_id
              LEFT JOIN ledger_members lm ON lm.userId = p.user_id AND lm.ledgerId = ${input.ledgerId}
        ...
        const participantMap = new Map<string, { userId: number; participantRole: string; displayName: string; avatar: string | null; matched: boolean }>();
        ...
        for (const p of participants) {
          if (!p.userId) continue; // 跳过没有绑定系统用户的参与方
          const key = `user_${p.userId}`;
          if (!participantMap.has(key)) {
            participantMap.set(key, {
              userId: Number(p.userId),
              participantRole: p.participantRole || 'participant',
              displayName: p.nickname || p.userName || p.username || `用户${p.userId}`,
    ...
    // ========== AJ 型定制账本（业务报销系统） ==========
    createCustomAJ: protectedProcedure
      .input(z.object({
        name: z.string().min(1).max(50),
        description: z.string().optional(),
      }))
      .mutation(async ({ ctx, input }) => {
        if (ctx.user.role !== 'super_admin' && ctx.user.role !== 'admin') {
          throw new TRPCError({ code: 'FORBIDDEN', message: '仅管理员可创建AJ定制账本' });
        }
        const ledger = await dbLedger.createLedger({
          name: input.name,
          description: input.description,
    ...
    listCustomAJ: protectedProcedure
      .query(async ({ ctx }) => {
        if (ctx.user.role !== 'super_admin' && ctx.user.role !== 'admin') {
          throw new TRPCError({ code: 'FORBIDDEN', message: '仅管理员可查看AJ定制账本列表' });
        }
        const db = await getDb();
        if (!db) throw new TRPCError({ code: 'INTERNAL_SERVER_ERROR', message: '数据库不可用' });
        const rows = await db
          ...
          .from(ledgers)
          .where(eq(ledgers.type, 'custom_aj'))
          .orderBy(desc(ledgers.createdAt));
        return rows;
      }),
    inviteToCustomAJ: protectedProcedure
      .input(z.object({
        ledgerId: z.number(),
        username: z.string(),
        role: z.enum(['member', 'admin', 'observer']).optional().default('member'),
      }))
      .mutation(async ({ ctx, input }) => {
        if (ctx.user.role !== 'super_admin' && ctx.user.role !== 'admin') {
          throw new TRPCError({ code: 'FORBIDDEN', message: '仅管理员可邀请成员加入AJ账本' });
        }
        const db = await getDb();
        if (!db) throw new TRPCError({ code: 'INTERNAL_SERVER_ERROR', message: '数据库不可用' });
        const [ledger] = await db
          ...
          .from(ledgers)
          .where(eq(ledgers.id, input.ledgerId));
        if (!ledger || ledger.type !== 'custom_aj') {
          throw new TRPCError({ code: 'BAD_REQUEST', message: '该账本不是AJ定制账本' });
        }
        return await dbLedger.inviteMemberByUsernameWithRole(input.ledgerId, ctx.user.id, input.username, input.role);
      }),
...
请严格按照以下JSON格式返回，不要有任何其他文字：
{
  ...
  /**
   * 涨停聚集效应：从 ts_bunching_stats 聚合全历史涨幅分布
   * 每个区间宽度 0.5%，范围 -11% ~ +11%，重点展示 ±10% 附近的聚集
   * 数据来源：ts_bunching_stats（每日由 stock-daily-scanner 写入，历史数据由 rebuild-bunching-stats.py 回填）
   */
  ...
  "gto.saveHand": protectedProcedure
    .input(z.object({
      ...
      turnMyAction: z.string().optional(),
      riverGtoAdvice: z.string().optional(),
      riverMyAction: z.string().optional(),
    }))
    .mutation(async ({ input, ctx }) => {
      const dbConn = await getDbConnection();
      if (!dbConn) throw new TRPCError({ code: 'INTERNAL_SERVER_ERROR', message: '数据库连接失败' });
      ...
      try {
        await dbConn.execute(`ALTER TABLE gto_hand_logs ADD COLUMN turn_gto_advice VARCHAR(100) DEFAULT '' AFTER flop_my_action`);
      } catch (_) {}
      try {
        await dbConn.execute(`ALTER TABLE gto_hand_logs ADD COLUMN turn_my_action VARCHAR(50) DEFAULT '' AFTER turn_gto_advice`);
      } catch (_) {}
      try {
        await dbConn.execute(`ALTER TABLE gto_hand_logs ADD COLUMN river_gto_advice VARCHAR(100) DEFAULT '' AFTER turn_my_action`);
      } catch (_) {}
      try {
