    ...
    funderCreateAssetOrder: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const insertResult = await db.execute(
          sql`INSERT INTO funder_asset_orders (order_no, ledger_id, user_id, coin, amount, buy_price, buy_date, buy_quantity, storage_account, admin_note, public_note, interest_rate_annual, interest_payment_type, interest_base, interest_base_currency, interest_rate_currency, interest_start_date, show_profit_share, collateral_assets, display_config, created_by)
              VALUES (${orderNo}, ${input.ledgerId}, ${input.userId}, ${input.coin}, ${input.amount}, ${input.buyPrice || null}, ${input.buyDate || null}, ${input.buyQuantity || null}, ${input.storageAccount || null}, ${input.adminNote || null}, ${input.publicNote || null}, ${input.interestRateAnnual || null}, ${input.interestPaymentType || null}, ${input.interestBase || null}, ${input.interestBaseCurrency || 'USDT'}, ${input.interestRateCurrency || 'USDT'}, ${input.interestStartDate || null}, ${input.showProfitShare !== false ? 1 : 0}, ${input.collateralAssets ? JSON.stringify(input.collateralAssets) : null}, ${input.displayConfig ? JSON.stringify(input.displayConfig) : null}, ${ctx.user.id})`
        ) as any;
        // 新订单创建后触发即时扫描
