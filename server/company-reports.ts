...
2. **企业标签之后的所有字段必须完整输出**：从 contactInfo 开始，到 shareholderInfo 结束，所有字段都必须输出
3. **空字段显示"暂无"**：如果某个字段在报告中没有找到或为空，输出"暂无"，不要省略字段
4. **保持 JSON 结构完整**：即使某个分类（如 financialData、riskInfo）下所有字段都为空，也要保留该分类，所有字段都显示"暂无"
5. 财务数据尽量提取最新年度的数据
6. 股东信息只提取前 5 大股东，如果没有股东信息，mainShareholders 输出空数组 []
7. 所有金额数据保留原始格式（如：1000万元）
8. 日期格式统一为 YYYY-MM-DD
