...
import { backfillEmbeddingAsync, buildItemEmbedText } from "./wecom-vector";
...
// ─── 收费项目同步到私有知识库 ─────────────────────────────────────────────────
// tenant_id -> wecom_knowledge_bases.id 映射（通过 wecom_clinic_channel 关联）
async function getPrivateKbIdForTenant(conn: any, tenantId: number): Promise<number | null> {
  try {
    const [rows]: any = await conn.execute(
      `SELECT wkb.id FROM wecom_knowledge_bases wkb
       INNER JOIN wecom_clinic_channel wcc ON wcc.clinic_channel_id = wkb.channel_id
       WHERE wcc.service_type = 'yaban' AND wcc.service_tenant_id = ? AND wkb.is_shared = 0
       LIMIT 1`,
      [tenantId]
    );
  ...
  try {
    ...
    for (const p of products) {
      ...
      if (p.price > 0 && p.price_max > 0 && p.price_max !== p.price) {
        priceStr = `${p.price}~${p.price_max}元/${p.unit || '次'}`;
      } else if (p.price > 0) {
        priceStr = `${p.price}元/${p.unit || '次'}`;
      } else {
        priceStr = `面议`;
      }
    ...
    // 删除旧的同步条目
    await conn.execute(
      `DELETE FROM wecom_knowledge_items WHERE kb_id = ? AND source_file = 'charge_products_sync'`,
      [kbId]
    );
    ...
    for (const [cat, items] of catMap.entries()) {
      ...
      const [r]: any = await conn.execute(
        `INSERT INTO wecom_knowledge_items (kb_id, item_type, question, answer, source_file, category) VALUES (?, 'qa', ?, ?, 'charge_products_sync', ?)`,
        [kbId, question, answer, cat]
      );
      const newId = (r as any).insertId;
      if (newId) {
        backfillEmbeddingAsync(conn, 'wecom_knowledge_items', Number(newId), buildItemEmbedText(question, answer));
      }
    }
  } catch (e: any) {
    console.error('[syncChargeToKb] 同步失败:', e?.message || e);
  }
}
...
const createInput = z.object({
  ...
  invoiceCompany: z.string().max(200).optional(),
  invoiceTitle: z.string().max(200).optional(),
  invoiceEmail: z.string().max(200).optional(),
  invoiceMobile: z.string().max(50).optional(),
  invoiceCompany2: z.string().max(200).optional(),
  invoiceTitle2: z.string().max(200).optional(),
  invoiceEmail2: z.string().max(200).optional(),
  invoiceMobile2: z.string().max(50).optional(),
  invoiceCompany3: z.string().max(200).optional(),
  invoiceTitle3: z.string().max(200).optional(),
  invoiceEmail3: z.string().max(200).optional(),
  invoiceMobile3: z.string().max(50).optional(),
  invoiceTaxNo1: z.string().max(100).optional(),
  invoiceTaxNo2: z.string().max(100).optional(),
  invoiceTaxNo3: z.string().max(100).optional(),
...
export const yabanCustomerRouter = router({
        ...
        (await (conn as any).execute(
          `INSERT INTO yaban_customer
            (tenant_id, name, gender, birthday, age, zodiac, chinese_zodiac, patient_type, medical_no, external_no, nickname,
             email, mobile, phone, region, address, license_plate, license_plate2, license_plate3,
            invoice_company, invoice_title, invoice_email, invoice_mobile,
            invoice_company2, invoice_title2, invoice_email2, invoice_mobile2,
            invoice_company3, invoice_title3, invoice_email3, invoice_mobile3,
            invoice_tax_no_1, invoice_tax_no_2, invoice_tax_no_3,
            avatar,
          ...
          [
            ...
            s(input.invoiceCompany),
            s(input.invoiceTitle),
            s(input.invoiceEmail),
            s(input.invoiceMobile),
            s(input.invoiceCompany2),
            s(input.invoiceTitle2),
            s(input.invoiceEmail2),
            s(input.invoiceMobile2),
            s(input.invoiceCompany3),
            s(input.invoiceTitle3),
            s(input.invoiceEmail3),
            s(input.invoiceMobile3),
            s(input.invoiceTaxNo1),
            s(input.invoiceTaxNo2),
            s(input.invoiceTaxNo3),
            s(input.avatar),
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      await (conn as any).execute(
        `UPDATE yaban_customer SET
           ...
           external_no = ?, nickname = ?, email = ?, mobile = ?, phone = ?, region = ?, address = ?, license_plate = ?, license_plate2 = ?, license_plate3 = ?,
           invoice_company = ?, invoice_title = ?, invoice_email = ?, invoice_mobile = ?,
           invoice_company2 = ?, invoice_title2 = ?, invoice_email2 = ?, invoice_mobile2 = ?,
           invoice_company3 = ?, invoice_title3 = ?, invoice_email3 = ?, invoice_mobile3 = ?,
           invoice_tax_no_1 = ?, invoice_tax_no_2 = ?, invoice_tax_no_3 = ?,
  ...
  executeChargeCopy: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // 异步同步目标诊所收费项目到私有知识库
      syncChargeProductsToKbAsync(conn, input.toTenantId).catch(() => {});
