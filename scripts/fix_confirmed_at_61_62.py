"""
修正订单 #61 和 #62 的 confirmed_at 为实际开仓日期 2025-10-15 00:00:00
"""
import sys
import pymysql
...
CORRECT_DATE = "2025-10-15 00:00:00"
ORDER_IDS = [61, 62]  # 正单 #61 + 赠与单 #62

with conn.cursor(pymysql.cursors.DictCursor) as cur:
    for order_id in ORDER_IDS:
        cur.execute(
            "SELECT id, confirmed_at, created_at, is_gift, coin FROM af_orders WHERE id = %s",
            (order_id,)
        )
        row = cur.fetchone()
        if not row:
        ...
        print("订单 #{} ({}): confirmed_at={}, created_at={}, is_gift={}".format(
            row['id'], row['coin'], row['confirmed_at'], row['created_at'], row['is_gift']))
