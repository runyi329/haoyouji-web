...
const MODE_LABELS: Record<string, { label: string; icon: string; color: string }> = {
 instant: { label: "", icon: "", color: "text-purple-400 bg-purple-500/10 border-purple-500/30" },
 scheduled: { label: "", icon: "⏰", color: "text-blue-400 bg-blue-500/10 border-blue-500/30" },
 milestone: { label: "", icon: "", color: "text-amber-400 bg-amber-500/10 border-amber-500/30" },
};

const STATUS_LABELS: Record<string, { label: string; color: string }> = {
 draft: { label: "", color: "text-gray-400 bg-gray-700/30 border-gray-600/30" },
 open: { label: "", color: "text-green-400 bg-green-500/10 border-green-500/30" },
 drawing: { label: "", color: "text-yellow-400 bg-yellow-500/10 border-yellow-500/30" },
 completed: { label: "", color: "text-gray-400 bg-gray-700/30 border-gray-600/30" },
 cancelled: { label: "", color: "text-red-400 bg-red-500/10 border-red-500/30" },
};
...
export default function LotteryList() {
 ...
 <div>
 <div className="font-bold text-amber-300 mb-1"></div>
 ...
 {isLoading && (
 <div className="text-center py-12 text-amber-400 animate-pulse">...</div>
 )}
 ...
 <div className="text-gray-500 text-sm mb-6">+ </div>
 <button
 onClick={() => navigate(`/lottery/create?ledgerId=${ledgerId}`)}
 ...
 <span className="text-gray-400"> {activity.participantCount} </span>
 {activity.winnerCount > 0 && (
 <span className="text-amber-400"> {activity.winnerCount} </span>
 )}
 </div>

 {activity.draw_at && activity.status === "open" && (
 <div className="mt-2 text-xs text-gray-500">
 {new Date(activity.draw_at).toLocaleString()}
 </div>
 )}
