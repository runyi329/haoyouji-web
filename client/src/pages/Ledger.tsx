                  ...
                  <div className="mb-3">
                    <div className="flex items-center justify-between gap-2 mb-1.5">
                      <div className="flex items-center gap-2 flex-1 min-w-0">
                        ...
                        {(ledger as any).userRole === 'owner' && (['custom_aa','custom_ab','custom_ac','custom_ad','custom_ae','custom_af','custom_ag','custom_ah','custom_ai'].includes((ledger as any).type)) && <span className="text-[10px] font-medium flex-shrink-0 px-1 py-0.5 rounded" style={{backgroundColor:'rgba(203,164,113,0.15)',color:'#9B7A3A'}}>{(ledger as any).type.replace('custom_','').toUpperCase()}</span>}
            ...
            <div className="space-y-2 max-h-96 overflow-y-auto">
              {ledgers && ledgers.length > 0 ? (
                ledgers.map((ledger) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      {(['custom_aa', 'custom_ab', 'custom_ac', 'custom_ad', 'custom_ae', 'custom_af', 'custom_ag', 'custom_ah', 'custom_ai', 'diet', 'opinion_book', 'opinion_book_demo'].includes((ledger as any).type)) ? <Gem className="w-5 h-5 text-[#CBA471]" strokeWidth={2} /> : <Notebook className="w-5 h-5 text-[#D32F2F]" />}
