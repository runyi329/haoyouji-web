...
export default function Ledger() {
    ...
    try {
      ...
      toast.success(
        <div className="flex flex-col gap-1">
          ...
          <div className="text-xs text-stable-gray">{filename}</div>
  ...
  return (
    <div className="min-h-screen bg-cream pb-24 max-w-md mx-auto relative shadow-2xl">
      {/* 顶部导航栏 */}
      <div className="bg-white-pure shadow-sm sticky top-0 z-40">
        <div className="px-4 py-3 flex items-center">
          <Link href="/">
            <button className="p-2 -ml-2 hover:bg-gray-100 rounded-lg transition-colors">
              <ChevronLeft className="w-6 h-6 text-core-black" />
            </button>
          </Link>
          <h1 className="flex-1 text-lg font-medium text-center text-core-black -ml-6">我的账本</h1>
        </div>
      </div>

      {/* 切换按钮 */}
      <div className="px-4 py-3">
        <div className="flex gap-2 bg-white-pure rounded-2xl p-1 shadow-sm">
          <button
      ...
      <div className="px-4 pb-4 space-y-3">
        {isLoading ? (
          ...
          filteredLedgers.map((ledger) => (
            ...
            >
              <div className="bg-white-pure rounded-2xl shadow-sm overflow-hidden hover:shadow-md transition-shadow">
                <div className="px-4 py-4">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {activeTab === "active" && (
                      <>
                        <button
                          className="text-xs h-8 rounded-xl bg-cream text-stable-gray font-medium hover:bg-gray-100 transition-colors"
                          onClick={(e) => {
                            e.stopPropagation();
                        ...
                        <button
                          className="text-xs h-8 rounded-xl bg-brand-red-light text-red-500 font-medium hover:bg-red-100 transition-colors"
                          onClick={(e) => {
                            e.stopPropagation();
                    ...
                    {activeTab === "archived" && (
                      <>
                        <button
                          className="text-xs h-8 rounded-xl bg-cream text-stable-gray font-medium hover:bg-gray-100 transition-colors col-span-2"
                          onClick={(e) => {
                            e.stopPropagation();
                            handleExport(ledger.id);
                          }}
      ...
      <Dialog open={showCreateDialog} onOpenChange={setShowCreateDialog}>
        <DialogContent className="w-[85%] rounded-lg p-0 gap-0" showCloseButton={false}>
          ...
          <button
            ...
            className="w-full text-center py-3.5 text-blue-500 font-medium border-b border-divider hover:bg-cream transition-colors"
          >
            新建全新账本
          </button>
          <button
          ...
          <button
            ...
            className="w-full text-center py-3.5 text-stable-gray font-medium hover:bg-cream transition-colors"
          >
            取消
          </button>
        </DialogContent>
      ...
      <Dialog open={showArchiveDialog} onOpenChange={setShowArchiveDialog}>
        <DialogContent className="w-[85%] rounded-lg p-0 gap-0" showCloseButton={false}>
          ...
          <div className="p-6 text-center">
            ...
            <p className="text-core-black mb-2 font-medium">一旦封存将不可以再修改</p>
            <p className="text-sm text-gray-500">封存后的账本只能查看和导出，无法继续编辑</p>
          </div>
          <button
            onClick={handleArchiveConfirm}
            disabled={archiveMutation.isPending}
            className="w-full text-center py-3.5 text-red-500 font-medium border-t border-divider hover:bg-cream transition-colors disabled:opacity-50"
          >
          ...
          <button
            ...
            disabled={archiveMutation.isPending}
            className="w-full text-center py-3.5 text-stable-gray font-medium border-t border-divider hover:bg-cream transition-colors disabled:opacity-50"
          >
            取消
          </button>
        </DialogContent>
            ...
            <div className="max-h-[300px] overflow-y-auto space-y-2">
              {searchUsername.length === 0 ? (
                ...
                searchResults.map((user: any) => (
                  <div
                    key={user.id}
                    className="flex items-center justify-between p-3 bg-cream rounded-lg hover:bg-gray-100 transition-colors"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      />
                      <div>
                        <p className="font-medium text-core-black">{user.name || user.username}</p>
      ...
      <Dialog open={showDestroyDialog} onOpenChange={setShowDestroyDialog}>
        <DialogContent className="w-[85%] rounded-lg p-0 gap-0" showCloseButton={false}>
          ...
          <div className="p-6 text-center">
            <div className="text-2xl mb-4">⚠️</div>
            <p className="text-core-black mb-2 font-medium">一旦销毁将永远消失</p>
          ...
          <button
            ...
            className="w-full text-center py-3.5 text-stable-gray font-medium border-t border-divider hover:bg-cream transition-colors"
          >
            取消
          </button>
        </DialogContent>
