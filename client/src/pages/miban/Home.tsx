      ...
      <section className="px-5 pt-6 pb-8">
        ...
        <Link href="/p/proj_hzxm2t/pear/tiangui">
          <div
            ...
            style={{ background: "#F4F4F4", height: 160 }}
          >
            {/* 大图：绝对定位，顶部对齐，不留白边 */}
            <img
            ...
            />
            {/* 地理标志图标：梨图片的左下角（梨图宽160px，图标贴着梨图左边缘底部） */}
            <img
            ...
            />
            {/* 顶部叶眉条：左上角和卡片共圆角，右下直角 */}
            {pearCountdown && (
              <div
                className="absolute top-0 left-0 flex items-center gap-2 px-4"
                style={{ background: "linear-gradient(to right, #FF6900 70%, rgba(255,105,0,0))", height: 32, zIndex: 4, width: "72%" }}
              >
            ...
            <div className="absolute flex flex-col justify-end pb-4 px-4" style={{ zIndex: 3, left: 0, top: 36, bottom: 0, width: "58%", boxSizing: "border-box" }}>
              {/* 商品名称 */}
              <h3 style={{ fontSize: 16, fontWeight: 700, color: "#111", lineHeight: 1.3, margin: 0, marginBottom: 3 }}>天桂梨</h3>
              {/* 认证文字行 */}
              <div style={{ marginBottom: 4 }}>
                <span style={{ fontSize: 9, color: "#4a9e5c", fontWeight: 600, letterSpacing: "0.02em" }}>地理标志产品 · 绿色食品</span>
              </div>
              {/* 产地标签行 */}
              <div style={{ display: "flex", alignItems: "center", gap: 4, marginBottom: 8 }}>
                <span style={{ fontSize: 10, color: "#888" }}>江西广丰 · 糖度 13%+</span>
              </div>
              {/* 价格行 */}
