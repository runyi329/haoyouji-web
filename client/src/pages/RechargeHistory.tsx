...
export default function RechargeHistory() {
  ...
  const balanceHistoryQuery = trpc.recharge.getBalanceHistory.useQuery({ limit: 50 });
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20">
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="bg-white rounded-lg overflow-hidden">
          <div className="px-4 py-3 border-b">
            <h2 className="font-semibold text-gray-900">余额变动</h2>
          </div>

          {balanceHistoryQuery.isLoading ? (
            <div className="p-8 text-center text-gray-500">加载中...</div>
          ) : !balanceHistoryQuery.data || balanceHistoryQuery.data.length === 0 ? (
            <div className="p-8 text-center text-gray-500">暂无余额变动记录</div>
          ) : (
            <div className="divide-y">
              {balanceHistoryQuery.data.map((record: any) => {
                const isPositive = parseFloat(record.amount) > 0;
                  ...
                  <div key={record.id} className="px-4 py-3">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className={`font-medium ${isPositive ? 'text-green-600' : 'text-red-600'}`}>
                        {isPositive ? '+' : ''}{parseFloat(record.amount).toFixed(2)} USDT
                      </span>
                    </div>
                    <div className="flex items-center justify-between text-xs text-gray-400">
                    ...
                    <div className="text-xs text-gray-400 mt-0.5">
                      余额: {parseFloat(record.balance).toFixed(2)} USDT
                    </div>
                  </div>
                );
              })}
