...
const ledgerTypeConfig: Record<string, { name: string; icon: any; defaultName: string }> = {
  ...
  diet: { name: "减肥账本", icon: Dumbbell, defaultName: "减肥打卡营" },
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="bg-white rounded-lg p-3 flex items-center gap-3">
          <div className="w-10 h-10 rounded-full bg-rose-100 flex items-center justify-center flex-shrink-0"
            style={{ color: isDiet ? '#e11d48' : 'var(--brand-red, #D32F2F)' }}>
        ...
        {/* 减肥账本说明 */}
        {isDiet && (
          <div className="bg-rose-50 border border-rose-100 rounded-lg p-3 text-xs text-rose-600 leading-relaxed">
            <span className="font-medium">💡 使用说明：</span>
            创建后邀请学员加入，在<span className="font-medium">学员管理</span>中为每位学员设置减肥档案，学员进入后即可看到自己的专属减肥面板。
          </div>
        )}

        {/* 账本名称 */}
        <div className="bg-white rounded-lg p-3 space-y-2">
          <Label htmlFor="ledgerName" className="text-sm text-gray-700">账本名称</Label>
          <Input
            id="ledgerName"
            value={ledgerName}
        ...
        {!isDiet && (
          <div className="bg-white rounded-lg p-3 space-y-2">
            <Label className="text-sm text-gray-700">账本结算货币</Label>
      ...
      <Dialog open={showSuccessDialog} onOpenChange={setShowSuccessDialog}>
        <DialogContent className="w-[85%] max-w-sm" showCloseButton={false}>
          <DialogTitle className="text-center text-base font-medium">
            {isDiet ? "减肥打卡营创建成功 🎉" : "账本创建成功了"}
          </DialogTitle>
          <DialogDescription className="text-center text-sm text-[#757575]">
            {isDiet
              ? "邀请学员加入后，在学员管理中为他们设置减肥档案"
              : "是否去设置或邀请好友加入账本？"}
          </DialogDescription>
          <DialogFooter className="flex-row gap-2 sm:justify-center">
            <Button variant="outline" onClick={handleSkip} className="flex-1 h-9">
              {isDiet ? "进入账本" : "不用了"}
            </Button>
            <Button
