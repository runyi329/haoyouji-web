    ...
    <div className="min-h-screen bg-[#F0F4F8] pb-24">
      <div className="text-white sticky top-0 z-20" style={{ background: BLUE_GRAD }}>
        <div className="flex items-center justify-between px-4 py-3">
          ...
          <div className="flex flex-col items-center">
            <span className="text-lg font-bold leading-tight">领用出库</span>
      ...
      <div className="px-4 pt-3 flex gap-2 overflow-x-auto no-scrollbar">
        <button onClick={() => setCategoryId(undefined)}
          className={`shrink-0 px-3 py-1 rounded-md text-xs ${categoryId === undefined ? "bg-sky-100 text-sky-600 font-medium" : "bg-white text-gray-400"}`}>全部</button>
        {(catsQuery.data || []).map((c) => (
          <button key={c.id} onClick={() => setCategoryId(c.id)}
            className={`shrink-0 px-3 py-1 rounded-md text-xs ${categoryId === c.id ? "bg-sky-100 text-sky-600 font-medium" : "bg-white text-gray-400"}`}>{c.name}</button>
        ))}
      </div>
        ...
        {listQuery.isLoading ? (
          ...
          <div className="space-y-2.5">
            {items.map((it) => {
              ...
              return (
                <div key={it.id} className="bg-white rounded shadow-sm p-3.5 flex items-center gap-3">
                  <div className="w-11 h-11 rounded-md bg-sky-50 flex items-center justify-center shrink-0">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    {q > 0 && (
                      <>
                        <button onClick={() => setQty(it.id, q - 1)} className="w-7 h-7 rounded-md border border-sky-400 text-sky-500 flex items-center justify-center"><Minus className="w-4 h-4" /></button>
                    ...
                    <button disabled={disabled || q >= it.stock} onClick={() => setQty(it.id, q + 1)}
                      className="w-7 h-7 rounded-md text-white flex items-center justify-center disabled:opacity-30" style={{ background: BLUE_GRAD }}>
      ...
      <div className="fixed bottom-0 inset-x-0 bg-white border-t border-gray-100 px-4 py-3 z-30 flex items-center gap-3">
        <button onClick={() => totalKinds > 0 && setShowCart(true)} className="relative">
          ...
          {totalKinds > 0 && <span className="absolute -top-1.5 -right-1.5 bg-red-500 text-white text-[10px] rounded-md w-4.5 h-4.5 px-1 flex items-center justify-center min-w-[18px] h-[18px]">{totalKinds}</span>}
        </button>
        ...
        <button onClick={() => totalKinds > 0 && setShowConfirm(true)} disabled={totalKinds === 0}
          className="px-6 py-2.5 rounded-md text-white font-medium disabled:opacity-40" style={{ background: BLUE_GRAD }}>去出库</button>
      </div>
              ...
              {cartLines.map((l) => (
                <div key={l.id} className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <button onClick={() => setQty(l.id, l.qty - 1)} className="w-7 h-7 rounded-md border border-sky-400 text-sky-500 flex items-center justify-center"><Minus className="w-4 h-4" /></button>
                    <span className="w-6 text-center text-sm font-bold">{l.qty}</span>
                    <button disabled={l.qty >= l.stock} onClick={() => setQty(l.id, l.qty + 1)} className="w-7 h-7 rounded-md text-white flex items-center justify-center disabled:opacity-30" style={{ background: BLUE_GRAD }}><Plus className="w-4 h-4" /></button>
                  </div>
                </div>
              ))}
            </div>
            <div className="px-5 py-3">
              <button onClick={() => { setShowCart(false); setShowConfirm(true); }} className="w-full py-3 rounded-md text-white font-medium" style={{ background: BLUE_GRAD }}>下一步</button>
            </div>
          </div>
        </div>
      )}
                ...
                <div className="flex gap-2">
                  {BIZ_TYPES.map((b) => (
                    <button key={b.key} onClick={() => setBizType(b.key)}
                      className={`px-4 py-1.5 rounded-md text-xs ${bizType === b.key ? "text-white" : "bg-gray-100 text-gray-500"}`}
                    ...
                    <div className="flex flex-wrap gap-2">
                      {(membersQuery.data || []).map((m) => (
                        <button key={m.userId} onClick={() => setReceiverId(receiverId === m.userId ? null : m.userId)}
                          className={`px-3 py-1.5 rounded-md text-xs ${receiverId === m.userId ? "bg-sky-500 text-white" : "bg-gray-100 text-gray-500"}`}>{m.name}</button>
                      ))}
                    </div>
                  </div>
                  <div>
