...
function ReviewsPanel() {
  ...
  return (
    <div className="px-3 pt-3 space-y-3">
      {reviews.map((r) => (
        <div key={r.id} className="bg-white rounded p-4">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className={`text-xs px-2 py-0.5 rounded-md ${r.status === 1 ? "bg-[#D1FAE5] text-[#059669]" : "bg-gray-100 text-gray-500"}`}
            >
              {r.status === 1 ? "已展示" : "已隐藏"}
            </button>
          </div>
          ...
          {r.images.length > 0 && (
            <div className="flex gap-2 mt-2 flex-wrap">
              {r.images.map((img: string, i: number) => (
                <img key={i} src={img} alt="晒单" className="w-14 h-14 rounded object-cover" />
              ))}
            </div>
          )}
          ...
          {replyId === r.id && (
            <div className="mt-2 space-y-2">
              ...
              <div className="flex gap-2">
                <button onClick={() => setReplyId(null)} className="flex-1 py-2 rounded-md bg-gray-100 text-gray-600 text-sm">取消</button>
                <button onClick={() => reply.mutate({ id: r.id, reply: replyText })} disabled={reply.isPending}
                  className="flex-1 py-2 rounded-md bg-[#2196C8] text-white text-sm disabled:opacity-50">保存回复</button>
              </div>
            </div>
          )}
        </div>
  ...
  return (
    <div className="px-3 pt-3 space-y-3">
      <button
        ...
        className="w-full py-2.5 rounded-md bg-[#2196C8] text-white text-sm font-medium flex items-center justify-center gap-1"
      >
      ...
      {isLoading ? (
        ...
        banners.map((b) => (
          <div key={b.id} className="bg-white rounded overflow-hidden">
        ...
        <div className="px-4 py-4 space-y-3" style={{ paddingBottom: "calc(24px + env(safe-area-inset-bottom, 0px))" }}>
          ...
          <div>
            ...
            {image ? (
              <div className="relative mt-2">
                <img src={image} alt="预览" className="w-full aspect-[2/1] object-cover rounded" />
                <label className="absolute bottom-2 right-2 px-3 py-1 rounded-md bg-black/50 text-white text-xs cursor-pointer">
                  更换
                  <input type="file" accept="image/*" className="hidden" onChange={handleFile} />
                </label>
              </div>
            ) : (
              <label className="mt-2 w-full aspect-[2/1] rounded border border-dashed border-gray-300 flex flex-col items-center justify-center text-gray-400 cursor-pointer">
          ...
          <div>
            ...
            <select value={linkType} onChange={(e) => setLinkType(e.target.value)}
              className="mt-1 w-full px-3 py-2 rounded border border-gray-200 text-sm bg-white">
          ...
          <button
            ...
            className="w-full py-3 rounded-md bg-gradient-to-r from-[#2196C8] to-[#3BA9E0] text-white text-sm font-semibold disabled:opacity-50"
          >
