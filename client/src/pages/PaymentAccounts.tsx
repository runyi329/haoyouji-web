...
export default function PaymentAccounts({ hideHeader = false }: PaymentAccountsProps = {}) {
  ...
  const handleSubmitWallet = () => {
    ...
    if (walletForm.walletType === "blockchain") {
      ...
      if (!/^\d{16,18}$/.test(walletForm.account.trim())) {
        toast.error("欧易 UID 必须为 16-18 位纯数字，请检查后重新输入");
        return;
      }
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20">
      ...
      {!hideHeader && (
        <div className="bg-white border-b border-gray-200 sticky top-0 z-10">
          <div className="flex items-center justify-between px-4 py-3">
            <button
              onClick={() => {
                const back = sessionStorage.getItem("payment_accounts_back");
                sessionStorage.removeItem("payment_accounts_back");
                navigate(back || "/parent/profile");
              }}
      ...
      {activeTab === "blockchain" && (
        <div className="p-4 space-y-3">
          {digitalWallets.filter(w => w.walletType === "blockchain" || w.walletType === "binance" || w.walletType === "okx").length === 0 ? (
            <div className="text-center py-12 text-gray-400">
              <Wallet className="w-16 h-16 mx-auto mb-3 opacity-30" />
              <p>暂无区块链钱包</p>
            </div>
          ) : (
            digitalWallets.filter(w => w.walletType === "blockchain" || w.walletType === "binance" || w.walletType === "okx").map((wallet) => (
              <div
                key={wallet.id}
                className="bg-white rounded-lg p-4 shadow-sm border border-gray-200"
              >
                ...
                <div className="space-y-2 mb-3">
                  ...
                  {(wallet.walletType === "binance" || wallet.walletType === "okx") && wallet.account && (
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-gray-600">{wallet.walletType === "binance" ? "币安 UID" : "欧易 UID"}</span>
                      <span className="text-sm font-mono">{wallet.account}</span>
                    </div>
                  )}
                  {wallet.accountName && (wallet.walletType === "binance" || wallet.walletType === "okx") && (
                    <div className="flex items-center justify-between">
              ...
              <>
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    placeholder="请输入 16-18 位欧易 UID"
                    maxLength={18}
                    inputMode="numeric"
                  />
                  <p className="text-xs text-gray-400">欧易 UID 为 16-18 位纯数字，可在欧易 App 个人中心查看</p>
                </div>
