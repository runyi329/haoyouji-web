...
export default function AfInviteTreePage() {
  ...
  // 账本基本信息（判断权限）
  const { data: ledgerData } = trpc.ledger.getById.useQuery({ id: ledgerId }, { enabled: !!ledgerId });
  ...
  // 邀请树数据
  const inviteTreeViewAsId = (isOwner || isAdmin) && (user as any)?.id !== YJH_USER_ID_CONST
    ? YJH_USER_ID_CONST
