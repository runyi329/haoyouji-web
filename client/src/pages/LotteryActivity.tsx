...
function SpinWheel({ prizes, winnerIdx, onDone }: { prizes: string[]; winnerIdx: number; onDone: () => void }) {
 ...
 // winnerIdx 
 const targetAngle = -(winnerIdx * sliceAngle + sliceAngle / 2) + Math.PI * 2 * 8; // 8
...
function FlipCards({ prizes, winnerIdx, onDone }: { prizes: string[]; winnerIdx: number; onDone: () => void }) {
 ...
 {flipped === idx ? (idx === winnerIdx ? prize : "") : ""}
 </span>
 </div>
 </div>
 </div>
...
export default function LotteryActivity() {
 ...
 if (!displayName.trim()) { setSignupError(""); return; }
 setSigningUp(true);
 setSignupError("");
 try {
 ...
 <div className="text-amber-400 text-lg animate-pulse">...</div>
 </div>
 );
 }
 ...
 const modeLabel = { instant: "", scheduled: "", milestone: "" }[activity.mode as string] ?? "";
 ...
 {isOpen ? "" : isCompleted ? "" : activity.status}
 </span>
 </div>
 ...
 {activity.max_participants && <span>/ {activity.max_participants} </span>}
 </div>
 </div>
 ...
 {[{ v: countdown.d, u: "" }, { v: countdown.h, u: "" }, { v: countdown.m, u: "" }, { v: countdown.s, u: "" }].map(({ v, u }) => (
 ...
 {/* */}
 <div className="mb-5">
 <h3 className="text-sm font-medium text-amber-200 mb-3"> </h3>
 ...
 {prize.is_consolation ? "" : `×${prize.quantity}`}
 </span>
 </div>
 ))}
 </div>
 ...
 <h3 className="font-bold mb-4 text-center"></h3>
 <input
 ...
 <p className="text-center text-xs text-gray-400 mt-2">¥{activity.signup_fee}</p>
 )}
 </div>
 )}
 ...
 <h3 className="font-bold text-amber-400 mb-6"> </h3>
 ...
 {drawResult.drawSeed.slice(0, 16)}...
 <span className="text-amber-600/60 ml-1"></span>
 </div>
 </div>
 )}
