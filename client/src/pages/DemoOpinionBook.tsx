...
function AIInsightsPanel({ ledgerId }: { ledgerId: number }) {
  ...
  const { data, isLoading, error } = trpc.opinionBook.aiInsights.useQuery(
    ...
    { enabled: enabled && ledgerId > 0, retry: 1, staleTime: 5 * 60 * 1000 }
  );
  ...
  const isAnalyzing = !enabled || isLoading;
    ...
    <>
      ...
      <div className="flex items-center justify-between flex-shrink-0 mb-1.5">
        <div className="flex items-center gap-1">
          ...
          <span className="text-xs font-semibold" style={{ color: 'rgba(255,255,255,0.9)' }}>AI 店长</span>
        </div>
        <button
          onClick={() => { setEnabled(true); setRefreshKey(k => k + 1); }}
          disabled={isAnalyzing}
          className="flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[10px] transition-opacity"
          style={{ backgroundColor: 'rgba(255,255,255,0.12)', color: 'rgba(255,255,255,0.7)', opacity: isAnalyzing ? 0.5 : 1 }}
        >
            ...
            <div className="flex gap-1">
              {[1,2,3].map(i => (
                <div key={i} className="w-1.5 h-1.5 rounded-full" style={{
                  backgroundColor: i <= dotCount ? 'rgba(255,255,255,0.85)' : 'rgba(255,255,255,0.2)',
                  transition: 'background-color 0.3s'
                }} />
              ))}
            </div>
            <span className="text-[10px]" style={{ color: 'rgba(255,255,255,0.6)' }}>AI 店长正在分析{'.··'.slice(0, dotCount)}</span>
          </div>
        ) : error ? (
