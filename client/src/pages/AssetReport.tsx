...
function ListItem({ 
  ...
  const getNumberColor = (idx: number) => {
    if (idx === 0) return "text-brand-red"; // 第1名：橙红色
    if (idx === 1) return "text-[#FF6347]"; // 第2名：番茄红
    if (idx === 2) return "text-brand-gold"; // 第3名：橙色
    return "text-gray-400"; // 其他：灰色
  };

  // 标签颜色
  const getTagColor = (tag?: string) => {
    if (tag === "热") return "bg-brand-red text-white";
    if (tag === "新") return "bg-brand-gold text-white";
      ...
      {isExpanded && (
        <div className="px-4 pb-4 border-t border-gray-100">
          ...
          <div className="mt-4 pt-4 border-t border-gray-100">
            <button
              onClick={handleShare}
              className="flex items-center justify-center space-x-2 w-full py-2.5 bg-brand-red hover:bg-brand-red-dark text-white rounded-lg transition-colors"
            >
              {copied ? (
                <>
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <Loader2 className="w-8 h-8 animate-spin text-brand-red-dark" />
      </div>
    );
  }
