...
export default function Recharge({ hideHeader = false, hideBalance = false, theme, onClose }: RechargeProps = {}) {
  ...
  if (isYaban) {
          ...
          <div className="p-4 space-y-4">
            ...
            <div className="rounded-2xl p-4 space-y-3 bg-white" style={{ boxShadow: "0 4px 16px rgba(33,150,200,0.1)" }}>
              ...
              <div className="flex justify-between items-center py-2 border-b border-gray-100">
                ...
                <span className="font-bold text-[#1E88D6]">{order.amount} USDT</span>
              </div>
              <div className="flex justify-between items-center py-2 border-b border-gray-100">
    ...
    if (order) {
      return (
        <div className="min-h-screen pb-20" style={{ background: "#F4F8FB" }}>
          {ybNav("确认支付", () => setOrder(null))}
          <div className="p-4 space-y-4">
            {timeLeft > 0 ? (
              <div className="rounded-xl p-4 flex items-start" style={{ background: "#FFF6E5", border: "1px solid #FFE0A3" }}>
                <Clock className="w-5 h-5 text-amber-500 mt-0.5 mr-3 flex-shrink-0" />
                <div>
                  <div className="font-medium text-amber-600">请在 {formatTime(timeLeft)} 内完成支付</div>
                  <div className="text-xs text-amber-500/80 mt-1">订单将在30分钟后自动过期</div>
                </div>
              </div>
            ) : (
              <div className="rounded-xl p-4 flex items-start" style={{ background: "#FDECEC", border: "1px solid #F7C5C5" }}>
                <AlertCircle className="w-5 h-5 text-red-400 mt-0.5 mr-3 flex-shrink-0" />
                <div>
                  <div className="font-medium text-red-500">订单已过期</div>
                  <div className="text-xs text-red-400/80 mt-1">请重新创建充值订单</div>
                </div>
              </div>
            )}
            <div className="rounded-2xl p-6 text-center bg-white" style={{ boxShadow: "0 4px 16px rgba(33,150,200,0.12)" }}>
              <div className="text-gray-400 text-sm mb-2">应付金额</div>
              <div className="text-5xl font-bold text-[#1E88D6] mb-1">{order.amount}</div>
              <div className="text-gray-400 text-sm">USDT ({order.network})</div>
            </div>
            <div className="rounded-2xl p-6 bg-white" style={{ boxShadow: "0 4px 16px rgba(33,150,200,0.1)" }}>
              <div className="text-center">
                <div className="text-sm text-gray-400 mb-4">扫码支付</div>
                {qrCode && (
                  <div className="inline-block p-3 rounded-xl bg-white border border-gray-100">
            ...
            <div className="rounded-2xl p-4 bg-white" style={{ boxShadow: "0 4px 16px rgba(33,150,200,0.1)" }}>
              ...
              <div className="flex items-center rounded-xl p-3" style={{ background: "#F4F8FB", border: "1px solid #E1ECF5" }}>
                <div className="flex-1 font-mono text-sm break-all mr-2 text-gray-700">{order.walletAddress}</div>
                <button onClick={() => copyToClipboard(order.walletAddress)} className="flex-shrink-0 p-2 rounded-lg" style={{ background: copied ? "rgba(34,197,94,0.12)" : "#EAF4FE" }}>
                  {copied ? <Check className="w-4 h-4 text-green-500" /> : <Copy className="w-4 h-4 text-[#1E88D6]" />}
                </button>
              </div>
            </div>
            <div className="rounded-xl p-4" style={{ background: "#FDECEC", border: "1px solid #F7C5C5" }}>
              <div className="text-sm font-medium text-red-500 mb-2">重要提示</div>
              <ul className="text-xs text-red-400 space-y-1.5">
                <li>· 如您使用OKX（欧易）提币，请务必选择「链上提币」方式</li>
                <li>· 若收款地址同为OKX用户，可能触发「内部转账」，此方式不走链上，系统将无法自动识别到账</li>
                <li>· 提币时如出现"内部转账"提示，请取消并改用链上转账</li>
              </ul>
            </div>
            <button onClick={handleSubmitTransfer} disabled={submitting || timeLeft === 0} className={blueBtn + " disabled:opacity-50"} style={blueBtnStyle}>
    ...
    // 主页面
    const networksY = ["TRC20", "ERC20", "BEP20", "APTOS", "SOLANA"] as const;
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="rounded-2xl p-4 bg-white" style={{ boxShadow: "0 4px 16px rgba(33,150,200,0.1)" }}>
            ...
            <div className="flex flex-wrap gap-2">
              {networksY.map(n => (
                <button key={n} onClick={() => setNetwork(n)} className="px-4 py-2 rounded-xl text-sm font-medium transition-all"
                  style={network === n ? { background: "linear-gradient(135deg,#2196C8,#1E88D6)", color: "#fff" } : { background: "#EAF4FE", color: "#5A7A92" }}>
                  {n}
                </button>
              ))}
            </div>
          ...
          <div className="rounded-2xl p-4 bg-white" style={{ boxShadow: "0 4px 16px rgba(33,150,200,0.1)" }}>
            ...
            <div className="flex items-center rounded-xl px-4 py-3 mb-3" style={{ background: "#F4F8FB", border: "1px solid #E1ECF5" }}>
              <input type="number" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="请输入充值金额"
                className="flex-1 min-w-0 text-xl font-bold outline-none bg-transparent text-gray-800 placeholder-gray-300" step="1" min="1" />
          ...
          <button onClick={handleCreateOrder} disabled={createOrderMutation.isPending || !amount} className={blueBtn + " disabled:opacity-50"} style={blueBtnStyle}>
