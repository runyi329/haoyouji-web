...
export default function FeedbackPage() {
  ...
  // 支付宝回调后显示简单返回页面（不判断成功失败，支付宝已告知用户结果）
  if (isPaid) {
    return (
      <div className="min-h-screen bg-[#F5F5F5] flex flex-col items-center justify-center px-6">
        <div className="bg-white rounded-2xl shadow-md p-8 w-full max-w-sm text-center">
          <p className="text-gray-500 text-sm mb-8">支付流程已完成</p>
          <button
            onClick={() => window.history.back()}
            className="w-full py-3 bg-[#D32F2F] text-white rounded-xl font-semibold text-base active:opacity-90"
          >
            返回
          </button>
        </div>
