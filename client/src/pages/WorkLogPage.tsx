  ...
  const renderCalendarView = () => {
    return (
      <div className="space-y-4">
        ...
        {/* 选中日期详情 */}
        {selectedDate && (() => {
          const logEntry = workLogData.find(e => e.date === selectedDate);
          return (
            <div className="bg-white rounded-2xl p-4 shadow-sm border border-red-100">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="bg-red-50 text-red-600 px-2.5 py-1 rounded-full text-xs font-bold">
                  {dayStats.get(selectedDate)?.count || 0} 次改动
                </span>
              </div>
              ...
              {logEntry ? (
                <div className="mb-4">
                  {logEntry.highlights && (
                    <div style={{ backgroundColor: '#fff8f8', border: '1px solid #fca5a5', borderRadius: 10, padding: '8px 12px', marginBottom: 10 }}>
                      <p style={{ color: '#D32F2F', fontSize: 12, fontWeight: 700, marginBottom: 2 }}>当日亮点</p>
                  ...
                  {dayStats.has(selectedDate) && (
                    <button
                      onClick={() => setDrawerDate(selectedDate)}
                      style={{ color: '#D32F2F', border: '1px solid #fca5a5', backgroundColor: '#fff' }}
                      className="mt-3 w-full py-2 rounded-xl text-xs font-semibold"
                    >
                      查看全部 {dayStats.get(selectedDate)?.count} 条提交记录
                    </button>
                  )}
                </div>
              ) : (
                dayStats.has(selectedDate) ? (
                  <div className="space-y-2">
                    {dayStats.get(selectedDate)?.commits.slice(0, 5).map((commit) => {
                      const typeInfo = getTypeInfo(commit.type);
                      return (
                        <div key={commit.hash} className="flex gap-2">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-700 leading-snug mt-0.5">{commit.cleanMessage}</p>
                          </div>
                        </div>
                      );
                    })}
                    {(dayStats.get(selectedDate)?.count || 0) > 5 && (
                      <button
                        onClick={() => setDrawerDate(selectedDate)}
                        style={{ color: '#D32F2F', border: '1px solid #fca5a5', backgroundColor: '#fff' }}
