...
function DeclarationPanel() {
  ...
  const handleUpload = () => {
    ...
    if (!uploadForm.textContent && !uploadForm.fileData) {
      toast.error("请上传文件或填写申报内容");
      return;
    }
  ...
  const handleEdit = (decl: any) => {
    ...
    setEditForm({
      ...
      cost: decl.cost ?? "",
      profit: decl.profit ?? "",
      taxAmount: decl.taxAmount ?? "",
      taxPaid: decl.taxPaid ?? "",
      notes: decl.notes ?? "",
      status: decl.status,
    });
  };
      ...
      <div className="flex flex-wrap gap-2 items-center">
        ...
        >
          <SelectTrigger className="w-52 h-8 text-sm">
            <SelectValue placeholder="选择企业（全部）" />
          </SelectTrigger>
          <SelectContent>
          ...
          <DialogContent className="max-w-md">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <Label className="text-xs">申报期间</Label>
                  <Input
                ...
                <div>
                  ...
                  <Select
                    value={uploadForm.declarationType}
                    onValueChange={(v) => setUploadForm(prev => ({ ...prev, declarationType: v }))}
                  >
                    <SelectTrigger className="h-8 text-sm mt-1">
                      <SelectValue />
                    </SelectTrigger>
              ...
              <div>
                ...
                <input
                  ...
                  className="mt-1 block w-full text-xs text-gray-500 file:mr-2 file:py-1 file:px-3 file:rounded-lg file:border-0 file:text-xs file:bg-blue-50 file:text-blue-600 hover:file:bg-blue-100"
                />
                {uploadForm.fileName && (
                  <p className="text-xs text-green-600 mt-1">已选择：{uploadForm.fileName}</p>
                )}
              </div>
              <div>
                <Label className="text-xs">或直接粘贴申报内容（文字）</Label>
                <Textarea
                  placeholder="将申报表文字内容粘贴到此处，AI 将自动提取财税数据..."
                  value={uploadForm.textContent}
                  onChange={(e) => setUploadForm(prev => ({ ...prev, textContent: e.target.value }))}
                  className="text-xs min-h-24 mt-1"
                />
              </div>
              <Button
        ...
        {selectedCompanyId && (
          <Dialog open={showSuggestionDialog} onOpenChange={setShowSuggestionDialog}>
            ...
            <DialogContent className="max-w-sm">
              <DialogHeader>
                <DialogTitle>添加成本票建议</DialogTitle>
              </DialogHeader>
              <div className="space-y-3">
                <div>
                  <Label className="text-xs">申报期间</Label>
                  <Input
                    placeholder="如 2026-06"
                    value={suggestionForm.period}
                    onChange={(e) => setSuggestionForm(prev => ({ ...prev, period: e.target.value }))}
                    className="h-8 text-sm mt-1"
                  />
                </div>
                <div>
                ...
                <div>
                  <Label className="text-xs">建议金额（元）</Label>
                  <Input
                    type="number"
                    placeholder="0.00"
                    value={suggestionForm.suggestedAmount}
                    onChange={(e) => setSuggestionForm(prev => ({ ...prev, suggestedAmount: e.target.value }))}
                    className="h-8 text-sm mt-1"
                  />
                </div>
                <div>
                  <Label className="text-xs">建议原因</Label>
                  <Textarea
                    placeholder="说明建议原因..."
                ...
                <Button
                  onClick={() => {
                    if (!suggestionForm.period) { toast.error("请填写申报期间"); return; }
                    addSuggestionMutation.mutate({ companyId: selectedCompanyId!, ...suggestionForm });
                  }}
            ...
            <div key={decl.id} className="bg-white border border-gray-100 rounded-xl p-4 shadow-sm">
                ...
                <div>
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span className="text-xs text-gray-400 bg-gray-50 px-2 py-0.5 rounded-full">
                      {decl.declarationType ?? "申报表"}
                    </span>
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                      decl.status === "accepted" ? "bg-green-50 text-green-600" :
                      decl.status === "rejected" ? "bg-red-50 text-red-600" :
                      decl.status === "submitted" ? "bg-blue-50 text-blue-600" :
                      "bg-gray-50 text-gray-500"
                    }`}>
                      {decl.status === "accepted" ? "已确认" :
                       decl.status === "rejected" ? "已拒绝" :
                       decl.status === "submitted" ? "已提交" : "草稿"}
                    </span>
                  </div>
                  {decl.notes && (
                    <p className="text-xs text-gray-400 mt-1">{decl.notes}</p>
                  )}
                </div>
                <button
                  onClick={() => editingId === decl.id ? setEditingId(null) : handleEdit(decl)}
              ...
              {editingId !== decl.id && (
                <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-3">
                  {[
                    { label: "收入", value: decl.revenue },
                    { label: "成本", value: decl.cost },
                    { label: "利润", value: decl.profit },
                    { label: "应纳税额", value: decl.taxAmount },
                    { label: "已缴税额", value: decl.taxPaid },
                  ].map(({ label, value }) => (
                    <div key={label} className="bg-gray-50 rounded-lg p-2">
                      <p className="text-xs text-gray-400">{label}</p>
                      <p className="text-sm font-semibold text-gray-700 mt-0.5">
                        {value != null ? `¥${Number(value).toLocaleString()}` : <span className="text-gray-300">-</span>}
                      </p>
                    </div>
                  ))}
                </div>
              ...
              {editingId === decl.id && (
                <div className="mt-3 space-y-3 border-t pt-3">
                  <div className="grid grid-cols-2 gap-2">
                    {[
                      { key: "revenue", label: "收入（元）" },
                      { key: "cost", label: "成本（元）" },
                      { key: "profit", label: "利润（元）" },
                      { key: "taxAmount", label: "应纳税额（元）" },
                      { key: "taxPaid", label: "已缴税额（元）" },
                    ].map(({ key, label }) => (
                      <div key={key}>
    ...
    <div className="space-y-4">
      ...
      <Tabs defaultValue="review" className="w-full">
        <TabsList className="flex w-full gap-1 h-auto mb-4">
          ...
          <TabsTrigger value="declarations" className="text-xs flex-1">
