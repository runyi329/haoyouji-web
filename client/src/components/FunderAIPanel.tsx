...
const ALERT_OPTIONS: { level: AlertLevel; label: string; desc: string; color: string }[] = [
  { level: 'none', label: '关闭预警', desc: '不发送任何邮件提醒', color: '#9CA3AF' },
...
export function FunderAIPanel({ orderId, ledgerId, orderInfo, onClose }: FunderAIPanelProps) {
  ...
  const handleSaveAlert = async (level: AlertLevel) => {
    if (!alertState?.userEmail) {
      toast.error('您尚未绑定邮箱，请先在个人中心绑定邮箱');
      return;
    }
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Tab 切换 */}
        <div className="flex mx-5 mb-4 rounded-xl overflow-hidden" style={{ background: '#F3F4F6' }}>
          {[
            ...
            <button
              ...
              style={{
                ...
                borderRadius: activeTab === tab.key ? '10px' : '0',
                boxShadow: activeTab === tab.key ? '0 1px 4px rgba(0,0,0,0.08)' : 'none',
                margin: activeTab === tab.key ? '3px' : '0',
              }}
                ...
                {ALERT_OPTIONS.map(opt => (
                  <button
                    ...
                    style={{
                      ...
                      opacity: !alertState?.userEmail && opt.level !== 'none' ? 0.5 : 1,
                    }}
                    onClick={() => handleSaveAlert(opt.level)}
                    disabled={saving || (!alertState?.userEmail && opt.level !== 'none')}
                  >
