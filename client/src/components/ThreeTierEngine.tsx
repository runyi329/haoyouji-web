...
export default function ThreeTierEngine({ leverageMultiplier }: ThreeTierEngineProps) {
  ...
  return (
    <div className="space-y-6">
        ...
        <div key={tier.id} className="space-y-3">
          ...
          <div className="px-1">
            <div className="flex items-baseline space-x-2">
              <h3 className="text-base font-bold text-[#424242]">{tier.name}</h3>
              <span className="text-xs text-[#757575] font-medium">{tier.subtitle}</span>
            </div>
            <p className="text-xs text-[#757575] mt-1">{tier.description}</p>
          </div>
            ...
            <div className="flex space-x-3 overflow-x-auto pb-2 scrollbar-hide">
              {tier.tasks.map((task) => (
                <div
                  ...
                  className="flex-shrink-0 w-32 bg-white rounded-xl p-3 border border-[#E0E0E0] hover:border-[#E0E0E0] hover:shadow-sm transition-all cursor-pointer"
                >
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-xs font-bold text-[#424242]">+{task.pu}</span>
                  </div>
                  <p className="text-xs font-semibold text-[#424242] mb-0.5">{task.title}</p>
                  <p className="text-[10px] text-[#757575] leading-tight">{task.description}</p>
                </div>
              ))}
            </div>
          )}
            ...
            <div className="space-y-2.5">
              {tier.tasks.map((task) => (
                <div
                  key={task.id}
                  onClick={() => handleTaskClick(task)}
                  className="bg-white rounded-xl p-4 shadow-sm hover:shadow-md transition-all cursor-pointer border border-[#E0E0E0]"
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center space-x-2 mb-1">
                        <h4 className="text-sm font-bold text-[#424242]">{task.title}</h4>
                        <div className="flex items-center space-x-1">
                          <span className="text-sm font-bold text-[#A80000]">+{task.pu}</span>
                          <span className="text-[10px] text-[#FFA726] font-semibold">×{task.leverageMultiplier.toFixed(2)}</span>
                        </div>
                      </div>
                      <p className="text-xs text-[#757575] mb-2">{task.description}</p>
                      
                      {task.frequency && (
                        <div className="flex items-center space-x-2 text-xs">
                          <span className="text-[#757575]">{task.frequency}</span>
                          {task.progress && (
                    ...
                    <ChevronRight className="w-4 h-4 text-[#757575] flex-shrink-0 ml-3" />
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="space-y-2.5">
              {tier.tasks.map((task) => (
                <div
                  key={task.id}
                  onClick={() => handleTaskClick(task)}
                  className="relative bg-gradient-to-br from-amber-50 to-orange-50 rounded-xl p-4 border border-[#FFA726] hover:border-[#FFA726] hover:shadow-md transition-all cursor-pointer overflow-hidden"
                >
                  {/* 高价值标签 */}
                  ...
                  <div className="flex items-start justify-between pr-16">
                    <div className="flex-1">
                      <div className="flex items-center space-x-2 mb-1">
                        {task.locked && <Lock className="w-3.5 h-3.5 text-[#757575]" />}
                        <h4 className="text-sm font-bold text-[#424242]">{task.title}</h4>
                        <div className="flex items-center space-x-1">
                          <span className="text-sm font-bold text-[#A80000]">+{task.pu}</span>
                          <span className="text-[10px] text-[#FFA726] font-semibold">×{task.leverageMultiplier.toFixed(2)}</span>
                      ...
                      {task.frequency && (
                        <span className="text-xs text-[#757575]">{task.frequency}</span>
                      )}
                      
                      {task.progress && (
                        <div className="mt-2">
                          <div className="flex items-center justify-between text-xs mb-1">
                            <span className="text-[#757575]">进度</span>
                            <span className="text-[#424242] font-medium">
                              {task.progress.current}/{task.progress.total} 天
                            </span>
                          </div>
                          <div className="w-full bg-[#FFA726]/50 rounded-full h-1.5">
                            <div
                    ...
                    <ChevronRight className="w-4 h-4 text-[#FFA726] flex-shrink-0 ml-3" />
                  </div>
                </div>
              ))}
            </div>
      ...
      {/* 增值路线图建议 */}
      <div className="bg-gradient-to-r from-white to-white rounded-xl p-4 border border-[#1976D2]">
        <div className="flex items-start space-x-3">
          <div className="w-8 h-8 rounded-lg bg-[#1976D2] flex items-center justify-center flex-shrink-0">
            <span className="text-white text-sm">💡</span>
          </div>
          <div className="flex-1">
            <h4 className="text-sm font-bold text-[#424242] mb-1">最强增值路径建议</h4>
            <p className="text-xs text-[#757575] leading-relaxed mb-2">
            ...
            <div className="mt-3 pt-3 border-t border-[#1976D2]">
              <p className="text-xs text-[#1976D2] font-semibold">
